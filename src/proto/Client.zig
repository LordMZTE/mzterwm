...
pub fn connect(io: std.Io, path: []const u8, read_buf: []u8, write_buf: []u8) !Client {
    const addr: std.Io.net.UnixAddress = try .init(path);
    const con = try addr.connect(io);
    const reader = con.reader(io, read_buf);
    const writer = con.writer(io, write_buf);
...
pub fn deinit(self: *const Client, io: std.Io) void {
    self.reader.stream.close(io);
}
